...
function ArchiveRow({ article }) {
   ...
   return (
      <motion.div
         className="grid-table bg-white dark:bg-primary-200 odd:bg-[#f8f8fa] dark:odd:bg-primary-100 transition-bg last:rounded-b-xl last:[&_img]:rounded-bl-xl"
         initial={{ opacity: 0 }}
         animate={{ opacity: 1 }}
         transition={{ duration: 0.3 }}
      >
         <img
            className="w-full 2k:h-28 h-25 2xl:h-23 xl:h-30 object-cover opacity-90 dark:opacity-75"
            src={image}
            alt={title}
         />
