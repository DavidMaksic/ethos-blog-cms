...
function SubmitButton({
   ...
   return (
      ...
      >
         <button
            className={`flex items-center gap-5 text-5xl px-10 pr-11 py-4 pt-5 text-white dark:text-accent-100/90 font-logo hover:text-accent/80 dark:hover:text-accent/70 transition-[color] duration-300 disabled:!cursor-default ${loadingTextStyle}`}
            type="submit"
            onClick={() => {
               if (onClick) onClick();
               if (handler) handler();
